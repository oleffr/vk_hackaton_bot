...
async def handle_date_input(event: MessageCreated):
    ...
    # Если пользователь в режиме свободного вопроса
    if current_mode == 'free_question':
        await event.message.answer(
            f"💬 *Режим свободного вопроса*\n\n"
            f"Ваш вопрос: \"{text}\"\n\n"
            f"🔍 *Эхо-ответ (в разработке):* {text}\n\n"
            f"Для выхода из режима используйте /cancel"
        )
        return
    
    # Если пользователь в режиме навигации
    if current_mode == 'navigation':
        await event.message.answer(
            f"🗺️ *Режим навигации*\n\n"
            f"Ваш запрос: \"{text}\"\n\n"
            f"📍 *Эхо-ответ (в разработке):* Ищу информацию по запросу: {text}\n\n"
            f"Для выхода из режима используйте /cancel"
        )
        return
